SWEP.PrintName = "MarumRTS controller"
...
local function GetUnitTrace()
	local tr = util.TraceLine( {
		...
		mask = MASK_WATER+MASK_SOLID,
		filter = function( ent )
			if ( ent:GetClass() == "ent_mrts_building" or ent:GetClass() == "ent_mrts_troop" ) then
				return true
			end
			return false
		end
...
function SWEP:Think()	
	...
	if (lastClick+0.05 < CurTime()) then
		...
		elseif (self.stage == STAGE_PLACING_TROOP) then // Placing troop
			...
			local canPlace, pos, message = MRTSCanPlaceTroop(GetNonUnitTrace(), troopData, troopSize, mrtsTeam)
			...
			if (self:PrimaryPressed()) then
				...
				if (checkForEnemies and not sandbox) then
					...
					if (canPlace) then
						...
						net.WriteTable(GetNonUnitTrace())
						net.SendToServer()
					end
				end
				self:SetClickCooldown()
		...
		elseif (self.stage == STAGE_PLACING_PART) then // Placing part
			...
			local canPlace, pos, message = MRTSCanPlacePart(tr, partData, partSize, mrtsTeam)
		...
		elseif (self.stage == STAGE_PLACING_BUILDING) then // Placing building
			...
			local canPlace, pos, message = MRTSCanPlaceBuilding(trace,buildingData,sizeVector,angle,mrtsTeam) -- Placeholder CanPlace
...
function SWEP:DrawHUD()
			...
			if (entTable.IsFOWVisibleToTeam(eyeEntity, mrtsTeam)) then
				...
				elseif (entTable.GetTeam(eyeEntity) == mrtsTeam) then
					if (entTable.GetUnderConstruction(eyeEntity)) then
						...
						if (data.makesContraptions) then
							local contraptionName = eyeEntity:GetNWString("contraptionName", "")
							if (contraptionName != "") then
								draw.SimpleTextOutlined( "Build "..contraptionName, "DermaDefault", ScrW()/2+16, ScrH()/2+60, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, outline_width, outline_color)
							else
								draw.SimpleTextOutlined( "Select contraption", "DermaDefault", ScrW()/2+16, ScrH()/2+60, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, outline_width, outline_color)
							end
							MRTSDrawIcon("gui/e.png", ScrW()/2, ScrH()/2+67, 16, 16)
						end
	...
	elseif (self.stage == STAGE_PLACING_TROOP) then
		...
		if (placingHint != "") then
			draw.SimpleTextOutlined(placingHint, "Trebuchet18", ScrW()/2, ScrH()/2+25, CAN_NOT_PLACE_COLOR, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, outline_width, outline_color)
		end
...
function MakeMRTSUnitButton(parent, h, unit, showInfo, onClick)
	...
	but.Paint = function(btn, w, h)
		...
		if (showInfo) then
			if (unit.cost) then
				MRTSDrawResources(0+w*0.45, 0, w*0.45, h, unit.cost)
			end
...
function SWEP:OpenGameMenu()
	...
	if (mrtsTeam >= 0) then
		...
		local playCategoriesSheet = vgui.Create( "DColumnSheet", playPanel )
		playCategoriesSheet:Dock(FILL)
		playCategoriesSheet:SetSize(150,0)
		local unitButtonSize = 34

		local categoryPanels = {}
		local function getCategoryPanel(category)
			if (categoryPanels[category] == nil) then
				categoryPanels[category] = vgui.Create( "DScrollPanel", playCategoriesSheet )
				categoryPanels[category]:Dock( FILL )
				categoryPanels[category]:SetSize(250, 0)
				categoryPanels[category].Paint = function( self, w, h )
					draw.RoundedBox( 4, 0, 0, w, h, Color( 30, 30, 30, self:GetAlpha() ) )
				end
				local sheet = playCategoriesSheet:AddSheet( category, categoryPanels[category] )
				sheet.Button:SetHeight(100)
			end
			return categoryPanels[category]
		end
		--------------------------------- Troops
		/*
		local troopsPanel = vgui.Create( "DScrollPanel", playCategoriesSheet )
		...
		*/
			...
			if (not v.unlisted) then
				...
				if (not blacklisted and (not v.factionSpecific or whitelisted)) then
					if (v.category != nil) then
						local but = MakeMRTSUnitButton(getCategoryPanel(v.category), unitButtonSize, v, true, function()
		...
		/*for k, v in pairs(categoryPanels) do
			local categorySheet = playCategoriesSheet:AddSheet( k, v )
			categorySheet.Button:SetHeight(100)
		end*/
	...
	if (mrtsTeam >= 0) then
		...
		disclaimer.Paint = function( self, w, h )
			draw.RoundedBox( 4, 0, 0, w, h, Color( 162, 31, 31) )
		end 
